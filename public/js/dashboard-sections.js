...
async function loadLevelsSection() {
  ...
  try {
    ...
    // Get all packages to check which levels user has
    const allPackages = packages;
    const userLevelNumber = parseInt(userCurrentLevel.replace('L', '') || '0');
    const purchasedLevels = userInvestments
      .filter(inv => inv.status === 'active')
      .map(inv => {
        const pkg = allPackages.find(p => p.id === inv.package_id);
        return pkg ? pkg.level : null;
      })
    ...
    levelsContent.innerHTML = `
      <p style="text-align: center; color: #6c757d; margin-bottom: 30px; max-width: 600px; margin-left: auto; margin-right: auto;">
        Select the investment level that suits your goals. Each level offers different daily returns. You can only purchase each level once and can only upgrade to higher levels.
      </p>
      <div style="display: grid; gap: 20px;">
        ${packages.map(pkg => {
          ...
          const levelNumber = parseInt(level.replace('L', '') || '0');
          const isPurchased = purchasedLevels.includes(level);
          const isDowngrade = levelNumber < userLevelNumber;
          const canPurchase = !isPurchased && !isDowngrade;
          
          return `
            <div class="card" style="display: grid; grid-template-columns: 100px 1fr 1fr auto; gap: 20px; align-items: center; ${!canPurchase ? 'opacity: 0.6;' : ''}">
              <div style="position: relative; width: 80px; height: 80px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; font-weight: bold; color: white; background: ${isPurchased ? 'linear-gradient(135deg, #28a745 0%, #20c997 100%)' : isDowngrade ? 'linear-gradient(135deg, #6c757d 0%, #5a6268 100%)' : 'linear-gradient(135deg, #007BFF 0%, #0056b3 100%)'};">
                ${level}
                ${isPurchased ? '<div style="position: absolute; top: -5px; right: -5px; background: #28a745; color: white; border-radius: 50%; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; font-size: 12px; border: 2px solid white;">✓</div>' : ''}
              </div>
              <div>
              ...
              <div style="display: flex; gap: 10px;">
                ${isPurchased ? `
                  <button class="btn" disabled style="flex: 1; background: #6c757d; cursor: not-allowed;">✓ Purchased</button>
                ...
                ` : isDowngrade ? `
                  <button class="btn" disabled style="flex: 1; background: #6c757d; cursor: not-allowed;">Cannot Downgrade</button>
                  <button class="btn" onclick="showDepositModal(${pkg.id}, ${amount})" style="flex: 1; background: #28a745; border-color: #28a745;">💰 Deposit</button>
                ` : `
                  <button class="btn" onclick="confirmInvestment(${pkg.id}, ${amount}, '${level}')" style="flex: 1;">Invest Now</button>
                  <button class="btn" onclick="showDepositModal(${pkg.id}, ${amount})" style="flex: 1; background: #28a745; border-color: #28a745;">💰 Deposit</button>
                `}
              </div>
            </div>
...
async function confirmInvestment(packageId, amount, level) {
  try {
    ...
    // Check if user already has investment in this level
    const investments = dashboardData.investments || [];
    ...
    if (targetPackage) {
      // Check if user already has investment in this level
      const hasInvestmentInLevel = investments.some(inv => {
        const invPackage = allPackages.find(p => p.id === inv.package_id);
        return invPackage && invPackage.level === targetPackage.level && inv.status === 'active';
      });

      if (hasInvestmentInLevel) {
        showAlert(`Invalid: You cannot purchase multiple investments. You already have an investment in ${targetPackage.level}. You can only upgrade to a higher level.`);
        return;
      }

      // Enforce sequential level progression - can only purchase the NEXT level
      const userLevelNumber = parseInt(userCurrentLevel.replace('L', '') || '0');
      const targetLevelNumber = parseInt(targetPackage.level.replace('L', '') || '0');
      const nextLevelNumber = userLevelNumber + 1;

      if (targetLevelNumber < userLevelNumber) {
        showAlert(`Invalid: You cannot purchase ${targetPackage.level}. Your current level is ${userCurrentLevel}. You can only purchase the next level (L${nextLevelNumber}).`);
        return;
      }
      
      if (targetLevelNumber > nextLevelNumber) {
        showAlert(`Invalid: You cannot skip levels. Your current level is ${userCurrentLevel}. You must purchase L${nextLevelNumber} first before purchasing ${targetPackage.level}.`);
        return;
      }
    }
    ...
    modal.innerHTML = `
      <div style="background: white; border-radius: 8px; padding: 30px; max-width: 500px; width: 90%; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        <h3 style="margin-bottom: 20px; color: #007BFF;">Confirm Investment</h3>
        <div style="margin-bottom: 20px;">
          <p style="margin-bottom: 10px; color: #333; font-size: 16px;">
            <strong>Confirm this amount based on the level you have chosen will be deducted from your account for the investment package:</strong>
          </p>
        ...
        <div style="margin-top: 20px; text-align: right;">
          ...
          <button onclick="processInvestment(${packageId}, ${amount})" class="btn">Confirm Investment</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
...
function copyDepositPhone() {
  ...
  if (phoneInput) {
    ...
    navigator.clipboard.writeText(phone).then(() => {
      ...
      console.error('Failed to copy:', err);
      showAlert('Failed to copy phone number. Please copy manually: ' + phone);
    });
  }
}
