...
app.delete('/api/admin/transactions', authenticateToken, requireAdmin, [
  ...
  try {
    ...
    console.log(`Marked ${ids.length} transaction(s) as deleted by admin ${req.user.userId} (balance preserved)`);
    res.json({ message: `Successfully deleted ${ids.length} transaction record(s). User balances remain intact.` });
  } catch (error) {
