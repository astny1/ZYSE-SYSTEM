...
app.post('/api/register', [
  ...
  try {
    ...
    const isAdmin = (process.env.ADMIN_PHONE && normalizedPhone === process.env.ADMIN_PHONE) || 
                    (normalizedEmail && normalizedEmail === process.env.ADMIN_EMAIL);
    console.log('Step 4: Admin check:', isAdmin);
...
app.post('/api/invest-from-balance', authenticateToken, [
  ...
  try {
    ...
    // Check if user already has an investment in this level (cannot purchase same level twice)
    const existingInvestmentInLevel = await dbGet(
      `SELECT i.id, i.package_id, p.level 
       FROM investments i 
       JOIN packages p ON i.package_id = p.id 
       WHERE i.user_id = ? AND p.level = ? AND i.status = 'active'`,
      [userId, packageData.level]
    );

    if (existingInvestmentInLevel) {
      return res.status(400).json({ 
        error: 'Invalid investment',
        message: `You cannot purchase ${packageData.level} again. You already have an active investment in this level.`
      });
    }

    // Enforce sequential level progression - can only purchase the NEXT level
    // User must be in level N to purchase level N+1 (cannot skip levels)
    const nextLevelNumber = userLevelNumber + 1;
    
    if (targetLevelNumber < userLevelNumber) {
      return res.status(400).json({ 
        error: 'Invalid investment',
        message: `You cannot purchase ${packageData.level}. Your current level is ${userCurrentLevel}. You can only purchase the next level (L${nextLevelNumber}).`
      });
    }
    ...
    if (targetLevelNumber > nextLevelNumber) {
      return res.status(400).json({ 
        error: 'Invalid investment',
        message: `You cannot skip levels. Your current level is ${userCurrentLevel}. You must purchase L${nextLevelNumber} first before purchasing ${packageData.level}.`
      });
    }
    
    // If targetLevelNumber === nextLevelNumber, that's correct - user is purchasing the next level
    ...
    if (user && user.email) {
      sendEmail(
        user.email,
        'Investment Confirmed',
        `Your investment of K${amount} in ${packageData.level} has been confirmed. The amount has been deducted from your account balance.`,
    ...
    console.log(`Investment created from balance: User ${userId}, Package ${packageId}, Amount K${amount}, Level ${packageData.level}`);

    res.json({
      message: 'Investment created successfully. Amount deducted from your balance.',
      investmentId: investResult.lastID,
