...
app.delete('/api/admin/deposits', authenticateToken, requireAdmin, [
  ...
  try {
    ...
    // Mark deposits as deleted instead of actually deleting them
    // This keeps the transactions intact so user balance remains correct
    // The deposits will be hidden from admin panel but balance calculations still include them
    const placeholders = ids.map(() => '?').join(',');
    
    // Mark investment records as deleted (we'll use a special status or add a deleted flag)
    // For now, we'll mark them with a special status 'deleted' 
    // Also mark associated transactions as deleted
    await dbRun(
      `UPDATE investments SET status = 'deleted' WHERE id IN (${placeholders}) AND transaction_txt IS NOT NULL AND transaction_txt != ''`,
      ids
    );
    
    // Mark associated transactions as deleted (not actually deleted, just flagged)
